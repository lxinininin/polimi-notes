...
* **Construction of an ND-TM from a Grammar G**:
	...
	1. **Input string on the input tape**: The input string $x$ (which we want to check if it belongs to the language $L(G)$ generated by grammar $G$ ) is initially placed on the Turing machine’s input tape.
	2. **Simulating Production Rules**:
		* The Turing machine uses a **memory tape** to store a sequence of symbols that represents the current state of the derivation. This sequence could be a mix of variables and terminals from the grammar $G$ .
		* The Turing machine then scans this memory tape to locate a substring that matches the **right-hand side (** $\beta$ **) of any production rule** $\alpha \rightarrow \beta$ in the grammar.
...
* **Derivation and Acceptance**:
  ...
  * **Right-to-Left Derivation**:
    ...
    * The Turing machine simulates the derivation relation (rewriting process) from **right to left**. This means it keeps applying production rules until it simplifies the string back to the start symbol $S$ , if possible.

  * **Acceptance Condition**:

  	* The string $x$ is accepted if the machine reaches a point where the memory tape holds only the **start symbol** $S$ . This indicates that $x$ can indeed be generated by starting with $S$ and applying production rules in $G$ .

  	* Formally, this is denoted as reaching a configuration where \( $\langle q_0, x \rangle \vdash^* \langle q_F, S \rangle$ \), meaning that the Turing machine transitions from the initial state with $x$ to a final state with $S$ on the tape.

  * **Non-Acceptance**:
  	* If no computation path leads to this configuration (with $S$ on the tape), the Turing machine will eventually reject $x$ , indicating that $x \notin L(G)$ .

* **Special Note on Monotonic Grammars**: 

  * For **monotonic grammars**, the Turing machine only needs the tape space initially occupied by $x$ .
  * This means the machine doesn’t need additional memory, which simplifies the computation. This is conceptually similar to **Linear Automata**, which operate with a limited tape length.



### Example

The grammar $G$ generates all strings of the form $x\$X$ , where

* $x$ is a string over the terminal alphabet $V_T$
* $\$$ is a separator symbol
* $X$ is a “copy” of $x$ but composed of nonterminal symbols. For instance, if $x = \text{aba}$ , then $x\$X$ would look like $\text{aba\$ABA}$ , where $A$ and $B$ are nonterminals corresponding to $a$ and $b$ , respectively
